<template>
  <div class="container">
        ...
        <el-popover placement="top-start" width="300" trigger="click">
          <div class="yiyuan-zhuyi-content">
            <p>
              ...
              <span
                style="color: #FF5566;"
                v-if="show.seckillPrice"
                v-html="'<price>¥</price>' + priceFormat(show.seckillPrice * seatList.length)"
              ></span>
              <span
                style="color: #FF5566;"
                v-else
            ...
            <p>1.{{cinema.cinemaName}}每次购票最多可选择{{buyNumLimit}}个座位；</p>
    ...
    <div class="updata-cont">
      ...
      <div
        class="updata-right"
        v-if="seatList.length > 0 && show.seckillPrice"
        @click="checkPhone"
        v-html="'<price>¥</price>' + priceFormat(show.seckillPrice * seatList.length) + '&nbsp;&nbsp;确认下单'"
      ></div>
      <div
        class="updata-right"
        v-if="seatList.length > 0 && !show.seckillPrice"
      ...
      <div>
        ...
        <form class="search-block" action="javascript:void 0">
          <input
            ...
            @blur="fixScroll"
            v-if="$init.channel.isUpdateCinemaVoucherPhone"
          />
          <input class="bind-phone-input" type="number" v-model="phone" disabled v-else />
        </form>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 手机号
      phone: this.$init.login.user.phone,
  ...
  created() {
    // 影院
    let localCinema = JSON.parse(
      window.localStorage.getItem(this.$init.key.cinemaSelectedCinema)
    );
    if (localCinema) {
    ...
    // 场次
    let localShow = JSON.parse(
      window.localStorage.getItem(this.$init.key.cinemaSelectedShow)
    );
    if (localShow) {
      this.show = localShow;
      if (this.$init.channel.buyNumLimit > 0) {
        this.buyNumLimit = this.$init.channel.buyNumLimit;
      } else {
        this.buyNumLimit = show.buyNumLimit;
      }
    } else {
      this.$router.back(-1);
    }
    ...
    clickSeat(value, val, action) {
      ...
      if ("del" == action) {
        ...
        if (1 == val.dataStatus) {
          ...
          // 添加
          if (addFlag) {
            if (vm.seatList.length < vm.buyNumLimit) {
              val.sectionId = value.id;
              vm.seatList.push(val);
            } else {
              vm.$toast("每次购票最多只能选择" + vm.buyNumLimit + "个座位！");
            }
          } else {
            delFlag = true;
          }
